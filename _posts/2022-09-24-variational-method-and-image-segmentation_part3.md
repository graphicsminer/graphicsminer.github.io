...
$$\begin{aligned}
    ...
    &= p( (I| \Omega_1) \cap  (I| \Omega_2) \cap \cdots \cap (I| \Omega_n)) \\
    &= \prod_{i = 1}^n p (I | \Omega_i)
\end{aligned}$$
...
$$p(I | \mathcal{P}(\Omega)) = \prod_{i = 1}^n \prod_{(x, y) \in \Omega_i} p (f_i(x, y))^{dx \,dy}$$
...
$$E(\mathcal{P}(\Omega), \{\theta_i\}) = -\sum_{i=1}^n\iint_{\Omega_i} \operatorname{log}(p(f_i(x, y)| \theta_i)) \,dx \, dy - \nu |C|$$
